...
# std: yoke-reference:continuous-integration.01
...
# std: yoke-reference:continuous-integration.02
check_toolchain_declared() {
  ...
  grep -qE 'go-version:[[:space:]]*.?[0-9]' "$workflow" || { echo "the language toolchain's version is not stated"; return 1; }
}
